...
某次听分享提到了图像的频域分析，旁边的人问我：什么是频域？什么是时域？一时感觉无法解释清楚，只是感觉和傅里叶级数有关系，为此找了些资料，学习一下时域和频域，完全参考了《傅里叶分析之掐死教程》{% ref 1 %}
...
></center>
>
>在数轴上有一个黑色的线段，它的长度是1。当它乘以3的时候，它的长度发生了变化，变成了橙色的线段，而当它乘以-1的时候，就变成了红色的线段，或者说线段在数轴上围绕原点旋转了180°。	
>
>乘-1其实就是乘了两次 $i$使线段旋转了180°，那么乘一次 $i$就是旋转了90°。
>
><center>
...
$$
这个公式关键的作用，是将正弦波统一成了简单的指数形式，{% hint '图像如下' '刷新网页查看' %}：
...
但不同的是，傅里叶变换出来的频谱不仅仅是{% hint '可见光' '不同光波的叠加' %}这样频率范围有限的叠加，而是频率从0到无穷所有频率的组合。
...
$$
$e^{i \theta}$可以理解为一条逆时针旋转的螺旋线，那么$e^{-i \theta}$可以理解为一条顺时针旋转的螺旋线。而$\cos \theta$则是这两条旋转方向不同的螺旋线叠加的一半（{% hint '因为这两条螺旋线的虚数部分相互抵消掉了' '相加时带有虚部的项和为0' %}）。这里，逆时针旋转的称为正频率，而顺时针旋转的称为负频率。
...
* 离散的：
  $$
  \begin{aligned}
  \text{fourier transform: }f(k) = \frac{1}{N}\sum^{N - 1}_{x = 0}{f(x)e^{-j2\pi kx / N}}, k = 0, 1, 2, \dots, N - 1 \\\\
  \text{inverse fourier transform: }f(k) = \sum^{N - 1}_{x = 0}{F(k)e^{j2\pi kx / N}}, x = 0, 1, 2, \dots, N - 1
  \end{aligned}
  $$
...
对图像进行傅里叶变换的原理如下公式：  
$$
F(u, v) = \sum^{H - 1}_{x = 0}\sum^{W - 1}_{y = 0}f(x, y)e^{}
$$
...
[2] <a href="关于傅里叶变换中的相位？ - 玉曼的回答 - 知乎 https://www.zhihu.com/question/23175691/answer/24579168">关于傅里叶变换中的相位</a>
