...
* `decltype(auto)` {% label warning @C++ 14 %}
  ...
  * `decltype(auto)`必须单独声明，不能结合指针、引用以及cv限定符
  ...
  auto x4a = { 1, 2 };              // x推导类型为std::initializer_list<int>
  decltype(auto) x4d = { 1, 2 };    // 编译失败，{ 1, 2 }不是表达式
  auto *x5a = &i;                   // x5a推导类型为int*
  decltype(auto) *x5d = &i;         // x编译失败，decltype(auto)必须单独声明
  ```
...
C++ 17之后编译器有做拷贝优化，虽然返回值是右值，函数返回类型是左值，因为做了拷贝优化所以不会报错
