...
* 用户<font color="#FF204E">**提供**</font>的构造函数和用户<font color="#FF204E">**声明**</font>的构造函数是有区别的
  ...
  * 所以在C++ 20 标准中禁止聚合类型使用用户声明的构造函数 {% label primary @C++ 20 %}
    * 用C++20环境编译后`X`和`Y`都不是聚合类型了
...
<center>
    <img src="65/c++-aggregate-type.png" />
</center>
...
* 重写（override）：在C++ 中是指派生类覆盖了基类的虚函数，这里的覆盖必须满足有相同的函数签名和返回类型，也就是说有相同的函数名、形参列表以及返回类型
...
* C++ 11 标准引入final说明符来阻止派生类去继承基类的虚函数 {% label info @C++ 11 %}
