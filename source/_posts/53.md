...
C++ 提供了一种方式可以直接得到类型的信息，定义在`<typeinfo>`，同样要限制使用，会造成性能上的问题。可以使用`typeid(xxx)`得到`const std::type_info`，类似于`sizeof`操作符。`type_info`是只读的，不能通过拷贝或者构造，只能通过`typeid`获得或者引用。可以使用`.name()`进行debug
...
* So to load shared library quickly, many procedures are omitted, which includes resolving different RTTI symbols 为了快速链接动态链接库，很多步骤都要被省略，其中也包括RTTI符号的合并

#### Const Cast

* It tries to drop the cv-qualifiers
  * 明确的知道了原来的对象就是非read-only，然后接收到的对象是`const`类型的
  * The author forgets the `const` in parameter, but it in fact doesn't write it
* `volatile` is similar 把`volatile`去掉会有cache的风险，`volatile`要求每次使用都要到内存中重新去读
* 一般情况下用不到
...
* Since C++ 17, `<variant>` and `<any>` are introduced to guarantee the safety
  ...
  * `std::any` can be seen as a `void*` with the original type "stored" magically, so that you'll fail to grasp(理解；领会) the inner object with the wrong type 可以加载任意的对象
    ...
    * `std::any` can have SBO (small buffer optimization) like `std::function` 编译器会给`std::any`在栈上分配比较小的buffer，存储一些非常小的对象，而不是在堆上分配内存
...
* Link, to locate symbols that are referred in other TUs 链接
...
* `#define` i.e. macros, which does pure text replacement
  ...
  * Parameters of macros are directly parsed by commas when no paratheses surrounded, e.g. `FUNC(SomeFunc<int, double>())` -> `SomeFunc<int + double>()` 遇到`,`就会替换
