...
已知重心坐标$\alpha$，$\beta$，$\gamma$的值求深度值$z$的公式如下（需要做重心坐标-插值校正[^1][^2]）：
$$
...
重心坐标是在2D空间里做的，不能用作插值3D空间的坐标，需要经过插值矫正才可以
...
[^1]: [透视投影-重心坐标-插值校正](https://zhuanlan.zhihu.com/p/380634903)
[^2]: [Perspective-Correct Interpolation](https://www.comp.nus.edu.sg/~lowkl/publications/lowk_persp_interp_techrep.pdf)