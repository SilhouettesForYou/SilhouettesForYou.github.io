...
* What is the contribution from each surface patch to $x$
  ...
  * Can be converted to the integration on the area of the patch
  ...
  $$
  \begin{aligned}
  L_o(p, \omega_0) &= \int_{\Omega_{patch}}{L_i(p, \omega_i)V(p, \omega_i)f_r(p, \omega_i, \omega_o)\cos{\theta_i}d\omega_i} \\\\
  &= \int_{A_{patch}}{L_i(p \rightarrow q)V(p, \omega_i)f_r(p, \omega_i, \omega_o)\frac{\cos{p}\cos{q}}{\Arrowvert p - q\Arrowvert^2}dA}
  \end{aligned}
  $$
...
* For a diffuse reflective path
  ...
  * Therefore
    $$
    E_p(x, n) = \Phi_p\frac{\max\\{0, \langle n_p\|x - x_p\rangle\\} \max\\{0, \langle n\|x_p - x \rangle\\}}{\Arrowvert x - x_p\Arrowvert^2}
    $$
